...
public class DashboardController {
    ...
    public String showAnalytics(Model model) {
        ...
        int overallUtilization = totalPossibleSlots == 0 ? 0 : (int) Math.round((double) usedSlots / totalPossibleSlots * 100);

        // Weekly utilization trends (Mon to Sat)
        Map<DayOfWeek, Integer> slotsPerDay = new HashMap<>();
        for (MasterRoutine r : routines) {
            if (r.getRoom() != null && r.getCourse() != null && r.getCourse().getSlotCount() != null && r.getDayOfWeek() != null) {
                slotsPerDay.put(r.getDayOfWeek(), slotsPerDay.getOrDefault(r.getDayOfWeek(), 0) + r.getCourse().getSlotCount());
            }
        }
        ...
        for (Department dept : departments) {
            ...
            int score = assignedHours == 0 ? 100 : (int) Math.round((double) (assignedHours - overbookedHours) / assignedHours * 100);
            ...
            if (dept.getName() != null) {
                if (dept.getName().toLowerCase().contains("computer")) icon = "fas fa-laptop-code text-blue-500";
                else if (dept.getName().toLowerCase().contains("elect")) icon = "fas fa-bolt text-yellow-500";
                else if (dept.getName().toLowerCase().contains("bio")) icon = "fas fa-dna text-purple-500";
                else if (dept.getName().toLowerCase().contains("phys")) icon = "fas fa-atom text-blue-400";
            }
            perf.put("icon", icon);
